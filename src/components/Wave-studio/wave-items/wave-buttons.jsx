  ...
  return (
    <div className="relative w-full h-[500px]">
        ...
        <div className="absolute top-[-20%] left-1/2 transform -translate-x-1/2"
          onClick={() => handleScrollAndSetActive('discovery')} style={{ cursor: 'pointer' }}
        >
            <DiscoverySvg />
        </div>

        <div className="absolute left-[-4%] top-1/2 transform -translate-y-1/2"
          onClick={() => handleScrollAndSetActive('tech')} style={{ cursor: 'pointer' }}
        >
            <TechSVG />
        </div>

        <div className="absolute right-[-4%] top-1/2 transform -translate-y-1/2"
          onClick={() => handleScrollAndSetActive('business')} style={{ cursor: 'pointer' }}
        >
            <BusinessSvg />
        </div>

        <div className="absolute bottom-[-20%] left-1/2 transform -translate-x-1/2"
          onClick={() => handleScrollAndSetActive('talent')} style={{ cursor: 'pointer' }}
        >
            <TalentSvg />
        </div>
    </div>
